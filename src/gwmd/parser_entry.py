...
（pure layer のみに依存）。sandbox-exec 下で network/fs-write を奪われた状態で動く。
    ...
    python -m gwmd.parser_entry <kind>   < raw  > ir.json
"""
...
def _parse(kind: str, raw: bytes):
    if kind in _GOOGLE:
        ...
        if kind == "gdocs":
            from gwmd.parsers import gdocs

            return gdocs.parse(data)
        if kind == "gsheet":
            from gwmd.parsers import gsheet

            return gsheet.parse(data)
        from gwmd.parsers import gslide

        return gslide.parse(data)
    if kind in _OOXML:
        if kind == "docx":
            from gwmd.parsers import docx
...
def main() -> None:
    if len(sys.argv) != 2:
        sys.stderr.write("usage: python -m gwmd.parser_entry <kind>\n")
        sys.exit(2)
